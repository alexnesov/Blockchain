...
class Wallet:
    ...
    Keeps track ofthe miner's balance.
    Allows a miner to authorizer transactions.
    """

    def __init__(self):
        self.address        = str(uuid.uuid4())[0:8]
        self.balance        = STARTING_BALANCE
        self.private_key    = ec.generate_private_key(ec.SECP256K1(),  # standard used by bitcoin
                                default_backend()
                                ) 
    ...
    def calculate_balance(blockchain, address):
        ...
        The balance is found by adding the output values that beling to the address since the most recent transaction by that address.
        """
        ...
        for block in blockchain.chain:
            for transaction in block.data:
                if transaction['input']['address'] == address:
                    # Any time the address conduts a new transaction it reset
                    # its balance
